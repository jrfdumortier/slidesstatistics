...
- In R, `var()` and `sd()` calculate the variance based on a sample, i.e., divide by $N-1$.
...
A statistic is a random variable (with its own probability distribution based) based on a sample. For example, repeated polling of 1,000 people about their political preferences will result in a different outcome each time. For the sampling distribution of the mean $\bar{x}$, we have the following:
...
Risk aversion for individuals as well as for firms. Why do insurance companies exist?
