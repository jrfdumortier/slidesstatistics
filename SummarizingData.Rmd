...
Skewed right (positive skewness): Long tail to the right
```{r,results=FALSE}
hist(skewness$beta28)
```

Symmetric
```{r,results=FALSE}
hist(skewness$beta55)
```

## Graphical Summaries: Empirical Distribution Function
```{r,echo=FALSE}
plot(ecdf(mh1$price),main="Empirical CDF of Meridian Hills Data")
```
